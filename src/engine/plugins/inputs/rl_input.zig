...
pub const InputPlugin = struct {
    ...
    pub const KeyVector2Map = struct {
        key: KeyboardKey,
        value: rl.Vector2,
    };

    pub fn mapKeysToVector2(
        self: *const Self,
        keymap: []const KeyVector2Map,
    ) rl.Vector2 {
        ...
        for (self.getKeysDown()) |key| {
            for (keymap) |km| {
                if (key == km.key) {
                    result = rl.Vector2Add(result, km.value);
                    break;
                }
            }
        }
...
pub const KeyboardKey = enum(usize) {
    ...
    fn getMaxKeyValue() usize {
        var max = std.math.minInt(usize);
        for (@typeInfo(KeyboardKey).Enum.fields) |f| {
            max = @max(f.value, max);
        }
        return max;
    }
